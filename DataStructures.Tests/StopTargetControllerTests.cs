...
namespace DataStructures.Tests
{
    public class PriceExitCalculatorTests
    {
        ...
        private void ShouldIncrementStopLong() {
            ...
            longstate.UpdateExits(stop.NewExit(longstate.TradeBuilder.ResultTimeline[^1], longstate.StopEntryTarget.CurrentExits, bars, 0));
            ...
            longstate.UpdateExits(stop.NewExit(longstate.TradeBuilder.ResultTimeline[^1], longstate.StopEntryTarget.CurrentExits, bars, 1));

            Assert.Equal(100, longstate.StopEntryTarget.StopPrice);
            Assert.Equal(double.NaN, longstate.StopEntryTarget.TargetPrice);
            
            longstate.Continue(bars[2]);
            longstate.UpdateExits(stop.NewExit(longstate.TradeBuilder.ResultTimeline[^1], longstate.StopEntryTarget.CurrentExits, bars, 2));

            Assert.Equal(101, longstate.StopEntryTarget.StopPrice);
            Assert.Equal(double.NaN, longstate.StopEntryTarget.TargetPrice);

            longstate.Continue(bars[3]);
            longstate.UpdateExits(stop.NewExit(longstate.TradeBuilder.ResultTimeline[^1], longstate.StopEntryTarget.CurrentExits, bars, 3));
        ...
        private void ShouldIncrementStopShort() {
            ...
            var shortState = new ShortTradeGenerator(0, new TradePrices(stop.InitialExit, 100), null);

            shortState.Continue(bars[0]);
            shortState.UpdateExits(stop.NewExit(shortState.TradeBuilder.ResultTimeline[^1], shortState.StopEntryTarget.CurrentExits, bars, 0));
            ...
            shortState.UpdateExits(stop.NewExit(shortState.TradeBuilder.ResultTimeline[^1], shortState.StopEntryTarget.CurrentExits, bars, 1));

            Assert.Equal(100, shortState.StopEntryTarget.StopPrice);
            Assert.Equal(double.NaN, shortState.StopEntryTarget.TargetPrice);

            shortState.Continue(bars[2]);
            shortState.UpdateExits(stop.NewExit(shortState.TradeBuilder.ResultTimeline[^1], shortState.StopEntryTarget.CurrentExits, bars, 2));

            Assert.Equal(99, shortState.StopEntryTarget.StopPrice);
            Assert.Equal(double.NaN, shortState.StopEntryTarget.TargetPrice);

            shortState.Continue(bars[3]);
            shortState.UpdateExits(stop.NewExit(shortState.TradeBuilder.ResultTimeline[^1], shortState.StopEntryTarget.CurrentExits, bars, 3));
