...
namespace DataStructures.Tests.Calculations
{
    public class ATRTests
    {
        ...
        [Fact]
        private void ShouldCalculateDeLoopedATRCorrectly() {
            var expected = new List<double>() {
                2,
                2.8,
                2.6399999999999997,
            ...
            for (int i = 0; i < myArray.Length; i++) {
                var lastClose = i > 0 ? myArray[i - 1].Close.Mid : 0;
                var lastAtr = i > 0 ? actuals[i - 1] : 0;
                actuals.Add(AverageTrueRange.Calculate(myArray[i], lastClose, lastAtr,5));
            }
            Asserters.ListDoublesEqual(expected, actuals);
        }
