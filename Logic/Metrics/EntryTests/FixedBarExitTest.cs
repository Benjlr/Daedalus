...
namespace Logic.Metrics.EntryTests
{
    ...
    class LongFixedBarExitTest : FixedBarExitTest
    {
        protected override void IterateBars(BidAskData[] data, int i) {
            ...
            if(BarsToWait + i < data.Length)
                _currentTrade.Exit(data[BarsToWait + i].Open.Ticks, data[BarsToWait + i].Open.Bid);
            else 
                _currentTrade.Exit(data.Last().Open.Ticks, data.Last().Close.Bid);
        }
    ...
    class ShortFixedBarExitTest : FixedBarExitTest
    {
        protected override void IterateBars(BidAskData[] data, int i) {
            ...
            if (BarsToWait + i  < data.Length)
                _currentTrade.Exit(data[BarsToWait + i].Open.Ticks, data[BarsToWait + i].Open.Ask);
            else
                _currentTrade.Exit(data.Last().Open.Ticks, data.Last().Close.Ask);
        }
